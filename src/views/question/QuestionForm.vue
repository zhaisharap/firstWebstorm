...
import { getArea, getQuestion } from '@/api/manage'
import { getFormFields } from '@/utils/g3common'
...
export default {
  ...
  created () {
    ...
    // 路由接受参数拼接
    this.description = `ID:${this.$route.query.id},标题：${this.$route.query.title}`
  },
  mounted () {
    ...
    const domFields = getFormFields(this.form.domFields)
    // const domFields = this.handleDomFields(this.form.domFields)
    // 获取表单详细信息
    getQuestion({ id: this.$route.query.id }).then(res => {
      ...
      const { form: { setFieldsValue } } = this
      const formData = pick(record, domFields)
      formData.buildTime = [moment(record.starttime), moment(record.endtime)]
      // formData.buildTime = [moment('2015-06-06', this.dateFormat), moment('2015-06-06', this.dateFormat)]
      this.$nextTick(() => {
